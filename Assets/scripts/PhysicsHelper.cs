...
namespace Ditzelgames
{
    public static class PhysicsHelper
    {
        ...
        public static void ApplyForceToReachVelocity(Rigidbody rigidbody, Vector3 velocidad, float force = 1, ForceMode mode = ForceMode.Force)
        {
            if (force == 0 || velocidad.magnitude == 0)
                return;

            velocidad = velocidad + velocidad.normalized * 0.2f * rigidbody.drag;
            ...
            {
                rigidbody.AddForce(velocidad * force, mode);
            }
            else
            {
                var velocityProjectedToTarget = (velocidad.normalized * Vector3.Dot(velocidad, rigidbody.velocity) / velocidad.magnitude);
                rigidbody.AddForce((velocidad - velocityProjectedToTarget) * force, mode);
            }
        }
